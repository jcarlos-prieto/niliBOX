...
Item {
    property var  box
    property var params;
    property real fwidth: 1 + Math.max(lserialport.width, laudiodevice.width, linputgain.width, llocalaudio.width)
    ...
    Flickable {
        ...
        TColumn {
            TGrid {
                ...
                TGrid {
                    ...
                    TLabel {
                        id: llocalaudio
                        height: b_unit
                        text: b_translate("Local audio:")
                    }
                    TCheck {
                        id: localaudio
                        width: b_unit
                        height: width
                        checked: params.localaudio === "true"
                        onClicked: b_send("localaudio", localaudio.checked)
                    }
                }
            }
            TLabel {
                ...
                visible: box ? box.os() === "windows" || box.os() === "macos" : false
                text: b_translate("If the virtual serial port is not recognized, try installing the driver for the CP210x UART bridge. Download it from: ") + "<a href='https://www.silabs.com/developer-tools/usb-to-uart-bridge-vcp-drivers?tab=downloads'>Silicon Labs</a>"
            ...
            TLabel {
                ...
                verticalAlignment: Text.AlignVCenter
                wrapMode: Text.WordWrap
                visible: box ? box.os() === "linux" : false
                text: b_translate("The USB audio device PCM2901 might not be present at boot. Please check that it is recognized by the operating system. Reconnecting the device may solve the issue.")
            }
        }
    }
