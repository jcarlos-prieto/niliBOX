...
Item {
    ...
    function b_start(params)
    {
        ...
        setScanLevel(b_getvar("scanlevel", "falses") === "true");
    ...
    function audioData(data)
    {
        ...
        data = box.DSP_scale(data, 4 * igain * igain);
    ...
    function setAFGain(value)
    {
        ...
        if (bPower.status === 1) {
            if (localaudio) {
                box.audioDevice_setVolume(audiodeviceid, 0);
                if (bMute.checked)
                    b_send("afgain", 0);
                else
                    b_send("afgain", value);
