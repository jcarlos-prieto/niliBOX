...
// SDRPLAY-RSP1A - CONFIG

import QtQuick
import QtQuick.Controls
import niliBOX.Controls
import niliBOX.Box
import "sdrplay.mjs" as SDRPLAY
...
Item {
    ...
    function refreshDevices()
    {
        ...
        let devicelist = SDRPLAY.list();
        if (devicelist.length === 0)
            return;

        for (let i = 0; i < devicelist.length; i++)
            if (devicelist[i].length > 0)
                devicemodel.append({value: devicelist[i], text: devicelist[i] + " - " + SDRPLAY.description(devicelist[i])});

        device.currentIndex = params.device ? device.find(params.device + " - " + SDRPLAY.description(params.device)) : 0;
